...
export const MotionSchema = z.object({
  reveal: z.object({
    by: z.enum(['word', 'char']).default('word'),
    stagger: z.number().min(0).default(0.07),
    duration: z.number().positive().default(0.7),
    dy: z.number().default(120),
  }).optional(),
  typewriter: z.object({
    duration: z.number().positive().default(1.6),
    delay: z.number().min(0).default(0),
  }).optional(),
  count: z.object({
    from: z.number().default(0),
    to: z.number(),
    duration: z.number().positive().default(1.4),
  ...
  pulse: z.object({
    from: z.number().positive().default(0.97),
    to: z.number().positive().default(1.03),
    rate: z.number().positive().default(1.6),
    axis: z.enum(['y', 'both']).default('both'),
  }).optional(),
  float: z.object({
    amp: z.number().default(6),
    rate: z.number().positive().default(2.4),
  }).optional(),
  press: z.object({
    at: z.number().min(0),
    dip: z.number().default(6),
  }).optional(),
  sleep: z.object({
    at: z.number().min(0),
    duration: z.number().positive().default(0.45),
  }).optional(),
...
const ElementBaseSchema = z.object({
  ...
  parameterizable: z.boolean().default(false),
...
export const TextElementSchema = ElementBaseSchema.extend({
  ...
  style: z.object({
    ...
    verticalAlign: z.enum(['flex-start', 'center', 'flex-end']).default('flex-start'),
    letterSpacing: z.number().default(0),
    lineHeight: z.number().positive().default(1.15),
    textMode: z.enum(['fit', 'overflow', 'truncate', 'flow']).default('fit'),
    opacity: z.number().min(0).max(1).default(1),
    backgroundColor: z.string().optional(),
    borderRadius: z.number().min(0).default(0),
    textStrokeWidth: z.number().min(0).default(0),
    textStrokeColor: z.string().default('#000000'),
  }).passthrough().default({}),
  repeatOnOverflow: z.boolean().default(false),
  flowZones: z.array(z.object({ y: z.number(), height: z.number().positive() })).optional(),
  flowBreakBehavior: z.enum(['line', 'word', 'char']).default('line'),
  flowMinLinesBeforeBreak: z.number().int().min(1).default(2),
...
export const ShapeElementSchema = ElementBaseSchema.extend({
  ...
  shapeType: z.enum(['rectangle', 'ellipse', 'line', 'arrow']).default('rectangle'),
  style: z.object({
...
export const WaveformElementSchema = ElementBaseSchema.extend({
  ...
  waveTrack: z.number().int().min(0).default(0),
  barCount: z.number().int().min(4).max(400).default(48),
  barGap: z.number().min(0).default(4),
  waveAlign: z.enum(['top', 'center', 'bottom']).default('center'),
  minBarHeight: z.number().min(0).default(2),
  borderRadius: z.number().min(0).default(2),
  style: z.object({ fill: z.string().default('#ffffff'), opacity: z.number().min(0).max(1).default(1) }).passthrough().default({}),
});

export const ContainerElementSchema = ElementBaseSchema.extend({
...
export const AudioTrackSchema = z.object({
  id: z.string().min(1),
  label: z.string().default('Audio'),
  url: z.string().optional(),
  filename: z.string().optional(),
  duration: z.number().positive().optional(),
  tts: z.object({
    text: z.string(),
    voiceId: z.string().default('en-us'),
    modelId: z.string().default('local'),
    parameterId: z.string().optional(),
  }).optional(),
  sfx: z.object({
    prompt: z.string(),
    seconds: z.number().min(1).max(22).optional(),
  }).optional(),
  volume: z.number().min(0).max(1).default(1),
...
export const SubtitleSchema = z.object({
  url: z.string().optional(),
  filename: z.string().optional(),
  source: z.enum(['upload', 'auto-audio', 'generated']).default('auto-audio'),
  enabled: z.boolean().default(true),
  cueCount: z.number().int().min(0).optional(),
  fontSize: z.string().default('44px'),
  fontFamily: z.string().default('Inter, sans-serif'),
  fontWeight: z.string().default('700'),
  color: z.string().default('#ffffff'),
  highlightColor: z.string().default('#D7FF64'),
  background: z.string().default('rgba(0,0,0,0.35)'),
  position: z.enum(['bottom', 'center', 'top']).default('bottom'),
  offset: z.number().min(0).default(120),
  mode: z.enum(['phrase', 'word-by-word']).default('word-by-word'),
  wordReveal: z.enum(['appear', 'highlight', 'align']).default('highlight'),
  split: z.enum(['word', 'letter']).default('word'),
  maxWordsPerLine: z.number().int().min(1).max(20).default(4),
  maxCharsPerLine: z.number().int().positive().optional(),
}).nullable();

export const PageMotionSchema = z.object({
  look: z.object({
    grain: z.number().min(0).max(0.2).default(0),
    vignette: z.number().min(0).max(0.6).default(0),
  }).optional(),
  camera: z.object({
    moves: z.array(z.object({
      ...
      ease: z.string().default('ease-in-out'),
    })).max(100),
  }).optional(),
}).passthrough();
...
export const TemplatePageSchema = z.object({
  ...
  canvas: z.object({
    width: z.number().int().min(10).max(5000),
    height: z.number().int().min(10).max(5000),
  ...
  elements: z.array(TemplateElementSchema).max(1000).default([]),
  ...
  videoDuration: z.number().min(0.1).max(120).nullable().default(null),
  motion: PageMotionSchema.optional(),
...
export const TemplateVariantSchema = z.object({
  ...
  width: z.number().int().min(10).max(5000),
  height: z.number().int().min(10).max(5000),
  mode: z.enum(['auto', 'edited', 'ai']).default('auto'),
...
export const TemplateDocumentSchema = z.object({
  ...
  canvasWidth: z.number().int().min(10).max(5000),
  canvasHeight: z.number().int().min(10).max(5000),
  pages: z.array(TemplatePageSchema).min(1).max(100),
  variants: z.array(TemplateVariantSchema).max(24).default([]),
});
